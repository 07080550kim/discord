...
const { initializeDatabase, userDB, messageDB, dmDB, fileDB, reactionDB, friendDB, serverDB, adminDB, callLogDB } = require('./database');
...
app.put('/api/user/username', authenticateToken, async (req, res) => {
    try {
        ...
        await userDB.updateUsername(req.user.id, username);
        res.json({ username });
    } catch (error) {
...
app.put('/api/messages/:messageId', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await messageDB.edit(req.params.messageId, content, req.user.id);
...
app.delete('/api/messages/:messageId', authenticateToken, async (req, res) => {
    try {
        const result = await messageDB.delete(req.params.messageId, req.user.id);
...
app.post('/api/messages/:messageId/pin', authenticateToken, async (req, res) => {
    try {
        await messageDB.pin(req.params.messageId);
        res.json({ success: true });
    } catch (error) {
...
app.delete('/api/messages/:messageId/pin', authenticateToken, async (req, res) => {
    try {
        await messageDB.unpin(req.params.messageId);
        res.json({ success: true });
    } catch (error) {
...
// Get pinned messages
app.get('/api/messages/:channelId/pinned', authenticateToken, async (req, res) => {
    try {
        const messages = await messageDB.getPinned(req.params.channelId);
        res.json(messages);
    } catch (error) {
...
// Search messages
app.get('/api/messages/:channelId/search', authenticateToken, async (req, res) => {
    try {
        const { q } = req.query;
        if (!q) {
        ...
        const messages = await messageDB.search(req.params.channelId, q);
        res.json(messages);
    } catch (error) {
...
app.get('/api/calls/history', authenticateToken, async (req, res) => {
    try {
        const history = await callLogDB.getHistory(req.user.id);
        res.json(history);
    } catch (error) {
...
app.get('/api/admin/logs', authenticateToken, async (req, res) => {
    try {
        const user = await userDB.findById(req.user.id);
        if (!userDB.isAdmin(user.username)) {
            return res.status(403).json({ error: 'Admin access required' });
        }
        
        const logs = await adminDB.getLogs();
        res.json(logs);
    } catch (error) {
...
app.post('/api/admin/ban', authenticateToken, async (req, res) => {
    try {
        ...
        await adminDB.logAction(req.user.id, 'ban', userId, reason);
...
app.post('/api/admin/unban', authenticateToken, async (req, res) => {
    try {
        ...
        await adminDB.logAction(req.user.id, 'unban', userId, '');
...
app.post('/api/admin/mute', authenticateToken, async (req, res) => {
    try {
        const user = await userDB.findById(req.user.id);
        if (!userDB.isAdmin(user.username)) {
            return res.status(403).json({ error: 'Admin access required' });
        ...
        await adminDB.muteUser(userId, req.user.id, reason, duration);
        await adminDB.logAction(req.user.id, 'mute', userId, `${reason} (${duration} min)`);
...
app.post('/api/admin/unmute', authenticateToken, async (req, res) => {
    try {
        const user = await userDB.findById(req.user.id);
        if (!userDB.isAdmin(user.username)) {
            return res.status(403).json({ error: 'Admin access required' });
        ...
        const { userId } = req.body;
        await adminDB.unmuteUser(userId);
        await adminDB.logAction(req.user.id, 'unmute', userId, '');
...
app.get('/api/admin/banned', authenticateToken, async (req, res) => {
    try {
        ...
        const banned = await adminDB.getBannedUsers();
        res.json(banned);
    } catch (error) {
...
app.get('/api/admin/muted', authenticateToken, async (req, res) => {
    try {
        const user = await userDB.findById(req.user.id);
        if (!userDB.isAdmin(user.username)) {
            return res.status(403).json({ error: 'Admin access required' });
        ...
        const muted = await adminDB.getMutedUsers();
        res.json(muted);
    } catch (error) {
...
app.get('/api/admin/calls', authenticateToken, async (req, res) => {
    try {
        const user = await userDB.findById(req.user.id);
        if (!userDB.isAdmin(user.username)) {
            return res.status(403).json({ error: 'Admin access required' });
        ...
        const calls = await callLogDB.getAllHistory();
        res.json(calls);
    } catch (error) {
        res.status(500).json({ error: 'Failed to get call history' });
    }
...
io.on('connection', async (socket) => {
    ...
    socket.on('send-message', async (messageData) => {
        try {
            const { channelId, message, replyTo } = messageData;
            
            // Check if user is muted
            const isMuted = await adminDB.isMuted(socket.userId);
            if (isMuted) {
                socket.emit('error-message', { error: 'You are muted' });
                return;
            }
    ...
    socket.on('edit-message', async (data) => {
        try {
            ...
            const result = await messageDB.edit(messageId, newContent, socket.userId);
    ...
    socket.on('delete-message', async (data) => {
        try {
            ...
            const result = await messageDB.delete(messageId, socket.userId);
    ...
    socket.on('unpin-message', async (data) => {
        try {
            const { messageId, channelId } = data;
            await messageDB.unpin(messageId);
    ...
    socket.on('call-started', async (data) => {
        try {
            ...
            const callLog = await callLogDB.create(socket.userId, receiverId, callType);
    ...
    socket.on('call-ended-log', async (data) => {
        try {
            const { duration } = data;
            if (socket.callLogId) {
                await callLogDB.endCall(socket.callLogId, duration);
            }
        } catch (error) {
            console.error('Call end log error:', error);
        }
    });

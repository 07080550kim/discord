...
function initializeApp() {
    ...
    if (typeof initializeAllNewFeatures === 'function') {
    ...
    if (socket && socket.connected) {
        if (currentView === 'dm' && currentDMUserId) {
            ...
            socket.emit('send-message', {
                channelId: channelId,
                message: message,
                replyTo: typeof replyToMessageId !== 'undefined' ? replyToMessageId : null
            });
        }
    }
...
function addMessageToUI(message) {
    ...
    if (message.replyTo) {
        ...
        replyDiv.innerHTML = `<span class="message-reply-author">Ответ на сообщение</span>`;
        content.appendChild(replyDiv);
    }
    
    const text = document.createElement('div');
    text.className = 'message-text';
    // Use parseMarkdown if available, otherwise plain text
    text.innerHTML = typeof parseMarkdown === 'function' ? parseMarkdown(message.text) : message.text;
    
    // Message actions (edit, delete, etc.)
    const actions = document.createElement('div');
    actions.className = 'message-actions';
    actions.innerHTML = `
        <button class="message-action-btn" onclick="replyToMessage(${message.id})">↩️</button>
        <button class="message-action-btn" onclick="editMessage(${message.id}, '${message.text.replace(/'/g, "\\'")}')">✏️</button>
        <button class="message-action-btn" onclick="deleteMessage(${message.id})">🗑️</button>
        <button class="message-action-btn" onclick="pinMessage(${message.id})">📌</button>
    `;
    ...
    if (message.author !== currentUser.username && typeof playNotificationSound === 'function') {
        playNotificationSound();
        if (typeof showDesktopNotification === 'function') {
            showDesktopNotification('Новое сообщение', `${message.author}: ${message.text}`);
        }
    }
}
...
// Initialize admin panel after socket connection
socket.on('connect', () => {
    console.log('Connected to server');
    setTimeout(() => {
        if (typeof initializeAdminPanel === 'function') {
